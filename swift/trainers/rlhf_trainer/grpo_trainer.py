    ...
    def _generate_and_score_completions(
        ...
        if self.args.use_vllm or self.args.use_lmdeploy:
            ...
            with unwrap_model_for_generation(self.model_wrapped, self.accelerator):
                ...
                #     outputs = self.engine.infer(inputs, self.request_config, use_tqdm=False, no_think_interval=0)
                # elif self.state.global_step > 200:
                #     outputs = self.engine.infer(inputs, self.request_config, use_tqdm=False, no_think_interval=2)
                # else:
                #     outputs = self.engine.infer(inputs, self.request_config, use_tqdm=False, no_think_interval=0)
                
                ## explore mode
                # if self.state.global_step > 1000:
                #     outputs = self.engine.infer(inputs, self.request_config, use_tqdm=False, explore_mode=False)
                # elif self.state.global_step > 200:
                #     outputs = self.engine.infer(inputs, self.request_config, use_tqdm=False, explore_mode=True)
                # else:
                #     outputs = self.engine.infer(inputs, self.request_config, use_tqdm=False, explore_mode=False)
        ...
        for i, (reward_func, reward_template) in enumerate(zip(self.reward_funcs, self.reward_templates)):
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                output_reward_func = reward_func(prompts, completions, **reward_kwargs)
