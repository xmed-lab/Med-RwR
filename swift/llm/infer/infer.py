...
class SwiftInfer(SwiftPipeline):
    ...
    def infer_dataset(self) -> List[Dict[str, Any]]:
        ...
        if request_config and request_config.stream:
            ...
            # for data, resp, labels, question_id in zip(val_dataset, resp_list, labels_list, question_ids):
            #     response = resp.choices[0].message.content
            #     data['messages'].append({'role': 'assistant', 'content': response})
            #     data = {'response': response, 'labels': labels, 'logprobs': resp.choices[0].logprobs, 'question_id': question_id, **data}
            #     result_list.append(data)
