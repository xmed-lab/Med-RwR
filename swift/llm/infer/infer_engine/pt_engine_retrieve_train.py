...
class PtEngine(InferEngine):
    ...
    def _prepare_generation_config(self, request_config: RequestConfig) -> _GenerationConfig:
        ...
        generation_config.top_logprobs = 11
    ...
    def preprocess_logits(self, batched_logits: Optional[List[torch.Tensor]], batched_generate_ids: torch.Tensor,
                          top_logprobs: int):
        ...
        for i in range(batch_size):
            ...
            for j, logits in enumerate(batched_logits):
                ...
                # Get token IDs for A,B,C,D,E,F,G,H,I,J using self.tokenizer
                target_letter_ids = []
                for letter in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']:
                    letter_tokens = self.tokenizer.encode(letter, add_special_tokens=False)
                    if letter_tokens:
                        target_letter_ids.extend(letter_tokens)
                # Remove duplicates while preserving order
                target_letter_ids = list(dict.fromkeys(target_letter_ids))
                
                tokens = [token] + target_letter_ids
    ...
    def _update_batched_logprobs(batched_logprobs: List[torch.Tensor], logits_streamer: Optional[LogitsStreamer],
        ...
        new_batched_logprobs = self.preprocess_logits(res, generate_ids[:, -seq_len:], top_logprobs)
    ...
    def _infer_full(self,
        ...
        for i in range(inputs['attention_mask'].shape[0]):
            ...
            for j in range(num_return_sequences):
                ...
                response = template.decode(generate_ids, template_inputs=template_inputs[i], skip_special_tokens=False)
        ...
        *,
        ...
        for i, batched_input in enumerate(batched_inputs):
            if response_ids is not None:
                for r, q, d in zip(response_ids[i], query_ids[i], doc_ids[i]):
